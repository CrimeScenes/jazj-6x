...
local resolverFactor = 0.05 -- Resolver adjustment factor
...
local function getHitboxOffset(distance)
    ...
    if distance <= 5 then
        return Vector3.new(zoneX * 0.5, zoneY * 0.5, zoneZ * 0.5)
    elseif distance <= 25 then
        return Vector3.new(zoneX * 0.75, zoneY * 0.75, zoneZ * 0.75)
    elseif distance <= 41 then
        return Vector3.new(zoneX, zoneY, zoneZ)
    else
        return Vector3.new(zoneX * 1.5, zoneY * 1.5, zoneZ * 1.5)
    end
end
...
local function predictTargetPosition(targetPlayer, deltaTime)
    ...
    if humanoidRootPart then
        ...
        local adjustedPredictionFactor = predictionFactor
        if distance > 41 then
            adjustedPredictionFactor = 0.3
        elseif distance > 25 then
            adjustedPredictionFactor = 0.2
        elseif distance > 5 then
            adjustedPredictionFactor = 0.1
        else
            adjustedPredictionFactor = 0.05
        end


        local prediction = humanoidRootPart.Position + velocity * deltaTime * adjustedPredictionFactor

    
        if #lastPositions > 0 then
            local lastPos = lastPositions[1]
            local delta = (prediction - lastPos).Magnitude
            if delta > 10 then
                prediction = prediction - velocity * resolverFactor
            end
        end
